...
pub fn create_phase_bat(position: PositionComponent, game: &mut Game) -> Entity {
    ...
    game.world.spawn((
        ...
        StatsComponent {
            ...
            luck: game.rng.gen_range(8, 10),
            agility: game.rng.gen_range(8, 12),
...
impl AI for PhaseBatAI {
    fn run(&mut self, this_entity: Entity, game: &mut Game) {
        ...
        if Self::try_attack(this_entity, self.chase_target, game) {
            ...
            game.world
                .get_mut::<StatsComponent>(this_entity)
                .unwrap()
                .agility += 2;
        } else {
...
impl PhaseBatAI {
    ...
    fn try_attack(this_entity: Entity, chase_target: Option<Entity>, game: &mut Game) -> bool {
        ...
        if let Some(target) = target {
            ...
            if !attack_missed {
                let minimum_damge = (this_stats.agility as f64 / 1.5).round() as u32;
