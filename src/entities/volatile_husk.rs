...
pub fn create_volatile_husk(position: PositionComponent, game: &mut Game) -> Entity {
        ...
        if let Some(target) = target {
            ...
            if (0..15).contains(&end_render_position.x) && (0..15).contains(&end_render_position.y)
            {
                game.animation_queue
                    .push_back(Box::new(VolatileHuskAttackAnimation {
                        time_started: None,
