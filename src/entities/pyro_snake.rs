...
impl AI for PyroSnakeAI {
    fn run(&mut self, this_entity: Entity, game: &mut Game) {
        ...
        if let Some(attack_target) = attack_result {
            ...
            {
                let old_burn_damage_left =
                    attack_target_combat.burn_debuff.0 * attack_target_combat.burn_debuff.1;
                if old_burn_damage_left < 12 {
                    attack_target_combat.burn_debuff = (3, 4);
                }
            }
        } else {
        ...
        if let Some(target) = target {
            ...
            if (0..15).contains(&end_render_position.x) && (0..15).contains(&end_render_position.y)
            {
                game.animation_queue
                    .push_back(Box::new(PyroSnakeAttackAnimation {
                        time_started: None,
