...
pub fn create_arcane_ooze(position: PositionComponent, game: &mut Game) -> Entity {
    let mut combat = CombatComponent::new(
...
impl AI for ArcaneOozeAI {
    fn run(&mut self, this_entity: Entity, game: &mut Game) {
        ...
        if let Some(attack_target) = attack_result {
            // Lower the target's agility and focus
            let mut attack_target_combat =
                game.ecs.get_mut::<CombatComponent>(attack_target).unwrap();
            attack_target_combat.agility_debuff.0 += 2;
            attack_target_combat.agility_debuff.1 = 15;
            attack_target_combat.focus_debuff.0 += 2;
            attack_target_combat.focus_debuff.1 = 15;
        } else {
        ...
        if let Some(target) = target {
            ...
            if (0..15).contains(&end_render_position.x) && (0..15).contains(&end_render_position.y)
            {
                game.animation_queue
                    .push_back(Box::new(ArcaneOozeAttackAnimation {
                        time_started: None,
...
impl Animation for ArcaneOozeAttackAnimation {
    ...
    fn is_complete(&self) -> bool {
        self.time_started.unwrap().elapsed() >= Duration::from_millis(200)
    }
}
